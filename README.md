...
## Results (latest snapshot: 2026-08-17 10:55 ET, SPY spot 775.01)

| Stage | Count |
|---|---|
| Contracts pulled (12 expiries) | 3,328 |
| Survive liquidity filter | 2,393 (72%) |
| IV solved via Newton | 1,588 (66%) |
| IV solved via Brent fallback | 569 (24%) |
| Failed (no root / outside bounds) | 236 (9.9%) |
...
The 236 failures cluster in the shortest expiries and deep-ITM strikes; 138 of them are quotes sitting below intrinsic value — prices that genuinely admit no implied vol, which the solver refuses rather than forcing a number.
...
Under Black-Scholes assumptions the IV-vs-strike line would be flat. Instead SPY shows the classic **equity skew**: the 04 Sep 2026 expiry prices 12% at the money vs 24% for puts 10% below spot (+12.6 vol pts). Three standard explanations, all pushing the same direction:
...
ATM IV currently averages 9.3% for expiries within a week vs nan% around one month: an **essentially flat** curve — neither calm-market contango nor stress backwardation. Flat term structures are transitional; watch whether the short end lifts (event risk approaching) or the long end reasserts the usual premium.
...
- **This snapshot uses live bid/ask midpoints** (market-hours pull): 386 zero-bid contracts dropped and 549 more for spreads wider than 20% of mid.
...
- **9.9% of IV solves failed** (236 contracts, mostly the shortest expiries and deep-ITM strikes); 138 were prices below intrinsic value — quotes that genuinely admit no implied vol.
