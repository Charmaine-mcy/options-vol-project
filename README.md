...
## Results (latest snapshot: 2026-09-03 13:58 ET, SPY spot 773.29)

| Stage | Count |
|---|---|
| Contracts pulled (12 expiries) | 3,834 |
| Survive liquidity filter | 2,993 (78%) |
| IV solved via Newton | 1,593 (53%) |
| IV solved via Brent fallback | 837 (28%) |
| Failed (no root / outside bounds) | 563 (18.8%) |
...
The 563 failures cluster in the shortest expiries and deep-ITM strikes; 199 of them are quotes sitting below intrinsic value — prices that genuinely admit no implied vol, which the solver refuses rather than forcing a number.
...
Under Black-Scholes assumptions the IV-vs-strike line would be flat. Instead SPY shows the classic **equity skew**: the 02 Oct 2026 expiry prices 12% at the money vs 22% for puts 10% below spot (+10.4 vol pts). Three standard explanations, all pushing the same direction:
...
ATM IV currently averages 10.2% for expiries within a week vs 11.9% around one month: an **upward-sloping (contango)** curve, the normal calm-market shape — near-term realized vol is expected to stay low, while longer horizons carry a risk premium for the things that haven't happened yet.
...
NFLX reports 20 Oct 2026 (47 days away) — the event is being priced *cross-sectionally* right now: the first post-earnings expiry (20 Nov) carries 38% ATM IV while later expiries decay back toward baseline as the one-day jump is diluted over more calendar time.
...
- **This snapshot uses live bid/ask midpoints** (market-hours pull): 436 zero-bid contracts dropped and 405 more for spreads wider than 20% of mid.
...
- **18.8% of IV solves failed** (563 contracts, mostly the shortest expiries and deep-ITM strikes); 199 were prices below intrinsic value — quotes that genuinely admit no implied vol.
