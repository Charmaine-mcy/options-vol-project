...
## Results (latest snapshot: 2026-07-24 11:45 ET, SPY spot 742.51)

| Stage | Count |
|---|---|
| Contracts pulled (12 expiries) | 2,982 |
| Survive liquidity filter | 2,211 (74%) |
| IV solved via Newton | 1,498 (68%) |
| IV solved via Brent fallback | 545 (25%) |
| Failed (no root / outside bounds) | 168 (7.6%) |
...
The 168 failures cluster in the shortest expiries and deep-ITM strikes; 157 of them are quotes sitting below intrinsic value — prices that genuinely admit no implied vol, which the solver refuses rather than forcing a number.
...
Under Black-Scholes assumptions the IV-vs-strike line would be flat. Instead SPY shows the classic **equity skew**: the 14 Aug 2026 expiry prices 15% at the money vs 26% for puts 10% below spot (+11.5 vol pts). Three standard explanations, all pushing the same direction:
...
ATM IV currently averages 13.2% for expiries within a week vs 14.7% around one month: an **upward-sloping (contango)** curve, the normal calm-market shape — near-term realized vol is expected to stay low, while longer horizons carry a risk premium for the things that haven't happened yet.
...
- **This snapshot uses live bid/ask midpoints** (market-hours pull): 476 zero-bid contracts dropped and 295 more for spreads wider than 20% of mid.
...
- **7.6% of IV solves failed** (168 contracts, mostly the shortest expiries and deep-ITM strikes); 157 were prices below intrinsic value — quotes that genuinely admit no implied vol.
