...
## Results (latest snapshot: 2026-08-18 10:08 ET, SPY spot 768.92)

| Stage | Count |
|---|---|
| Contracts pulled (12 expiries) | 3,396 |
| Survive liquidity filter | 2,568 (76%) |
| IV solved via Newton | 1,384 (54%) |
| IV solved via Brent fallback | 690 (27%) |
| Failed (no root / outside bounds) | 494 (19.2%) |
...
The 494 failures cluster in the shortest expiries and deep-ITM strikes; 248 of them are quotes sitting below intrinsic value — prices that genuinely admit no implied vol, which the solver refuses rather than forcing a number.
...
Under Black-Scholes assumptions the IV-vs-strike line would be flat. Instead SPY shows the classic **equity skew**: the 11 Sep 2026 expiry prices 13% at the money vs 24% for puts 10% below spot (+11.5 vol pts). Three standard explanations, all pushing the same direction:
...
ATM IV currently averages 12.0% for expiries within a week vs 12.5% around one month: an **upward-sloping (contango)** curve, the normal calm-market shape — near-term realized vol is expected to stay low, while longer horizons carry a risk premium for the things that haven't happened yet.
...
- **This snapshot uses live bid/ask midpoints** (market-hours pull): 324 zero-bid contracts dropped and 504 more for spreads wider than 20% of mid.
...
- **19.2% of IV solves failed** (494 contracts, mostly the shortest expiries and deep-ITM strikes); 248 were prices below intrinsic value — quotes that genuinely admit no implied vol.
