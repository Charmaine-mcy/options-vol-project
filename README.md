...
## Results (latest snapshot: 2026-08-07 14:13 ET, SPY spot 771.79)

| Stage | Count |
|---|---|
| Contracts pulled (12 expiries) | 3,486 |
| Survive liquidity filter | 2,780 (80%) |
| IV solved via Newton | 1,730 (62%) |
| IV solved via Brent fallback | 947 (34%) |
| Failed (no root / outside bounds) | 103 (3.7%) |
...
The 103 failures cluster in the shortest expiries and deep-ITM strikes; 73 of them are quotes sitting below intrinsic value — prices that genuinely admit no implied vol, which the solver refuses rather than forcing a number.
...
Under Black-Scholes assumptions the IV-vs-strike line would be flat. Instead SPY shows the classic **equity skew**: the 04 Sep 2026 expiry prices 13% at the money vs 22% for puts 10% below spot (+9.5 vol pts). Three standard explanations, all pushing the same direction:
...
ATM IV currently averages 10.8% for expiries within a week vs 12.6% around one month: an **upward-sloping (contango)** curve, the normal calm-market shape — near-term realized vol is expected to stay low, while longer horizons carry a risk premium for the things that haven't happened yet.
...
- **This snapshot uses live bid/ask midpoints** (market-hours pull): 304 zero-bid contracts dropped and 402 more for spreads wider than 20% of mid.
...
- **3.7% of IV solves failed** (103 contracts, mostly the shortest expiries and deep-ITM strikes); 73 were prices below intrinsic value — quotes that genuinely admit no implied vol.
