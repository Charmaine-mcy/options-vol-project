...
## Results (latest snapshot: 2026-07-22 12:30 ET, SPY spot 749.80)

| Stage | Count |
|---|---|
| Contracts pulled (12 expiries) | 3,199 |
| Survive liquidity filter | 2,404 (75%) |
| IV solved via Newton | 1,610 (67%) |
| IV solved via Brent fallback | 708 (29%) |
| Failed (no root / outside bounds) | 86 (3.6%) |
...
The 86 failures cluster in the shortest expiries and deep-ITM strikes; 75 of them are quotes sitting below intrinsic value — prices that genuinely admit no implied vol, which the solver refuses rather than forcing a number.
...
Under Black-Scholes assumptions the IV-vs-strike line would be flat. Instead SPY shows the classic **equity skew**: the 21 Aug 2026 expiry prices 14% at the money vs 24% for puts 10% below spot (+10.4 vol pts). Three standard explanations, all pushing the same direction:
...
- **This snapshot uses live bid/ask midpoints** (market-hours pull): 414 zero-bid contracts dropped and 381 more for spreads wider than 20% of mid.
...
- **3.6% of IV solves failed** (86 contracts, mostly the shortest expiries and deep-ITM strikes); 75 were prices below intrinsic value — quotes that genuinely admit no implied vol.
