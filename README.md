...
## Results (latest snapshot: 2026-08-13 11:31 ET, SPY spot 776.15)

| Stage | Count |
|---|---|
| Contracts pulled (12 expiries) | 3,937 |
| Survive liquidity filter | 3,064 (78%) |
| IV solved via Newton | 1,980 (65%) |
| IV solved via Brent fallback | 880 (29%) |
| Failed (no root / outside bounds) | 204 (6.7%) |
...
The 204 failures cluster in the shortest expiries and deep-ITM strikes; 204 of them are quotes sitting below intrinsic value — prices that genuinely admit no implied vol, which the solver refuses rather than forcing a number.
...
Under Black-Scholes assumptions the IV-vs-strike line would be flat. Instead SPY shows the classic **equity skew**: the 11 Sep 2026 expiry prices 12% at the money vs 22% for puts 10% below spot (+9.6 vol pts). Three standard explanations, all pushing the same direction:
...
Put-call parity says call IV and put IV should be identical at the same strike — but in this snapshot they differ by 2.2 vol points even at the money (13.5% calls vs 11.3% puts on the 11 Sep 2026 expiry), larger than parity should allow. The likely culprit is quote noise and wide ITM spreads; treat single-strike IV readings from this snapshot with caution. The structural ITM divergence is on top of that:
...
ATM IV currently averages 10.7% for expiries within a week vs 12.5% around one month: an **upward-sloping (contango)** curve, the normal calm-market shape — near-term realized vol is expected to stay low, while longer horizons carry a risk premium for the things that haven't happened yet.
...
No earnings inside the tracking window at the moment — the next report is 20 Oct 2026 (~68 days away); tracking resumes automatically inside the 60-day window. The chart shows the **archived analysis of the 16 Jul 2026 event**, rendered from numbers persisted at capture time (`data/earnings_archive_NFLX.json`), not recomputed:
...
- **This snapshot uses live bid/ask midpoints** (market-hours pull): 417 zero-bid contracts dropped and 456 more for spreads wider than 20% of mid.
...
- **6.7% of IV solves failed** (204 contracts, mostly the shortest expiries and deep-ITM strikes); 204 were prices below intrinsic value — quotes that genuinely admit no implied vol.
