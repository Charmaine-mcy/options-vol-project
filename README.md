...
## Results (latest snapshot: 2026-09-08 14:01 ET, SPY spot 767.51)

| Stage | Count |
|---|---|
| Contracts pulled (12 expiries) | 3,026 |
| Survive liquidity filter | 2,285 (76%) |
| IV solved via Newton | 1,283 (56%) |
| IV solved via Brent fallback | 635 (28%) |
| Failed (no root / outside bounds) | 367 (16.1%) |
...
The 367 failures cluster in the shortest expiries and deep-ITM strikes; 157 of them are quotes sitting below intrinsic value — prices that genuinely admit no implied vol, which the solver refuses rather than forcing a number.
...
Under Black-Scholes assumptions the IV-vs-strike line would be flat. Instead SPY shows the classic **equity skew**: the 02 Oct 2026 expiry prices 12% at the money vs 24% for puts 10% below spot (+11.5 vol pts). Three standard explanations, all pushing the same direction:
...
Put-call parity says call IV and put IV should be identical at the same strike — but in this snapshot they differ by 1.7 vol points even at the money (11.5% calls vs 13.1% puts on the 02 Oct 2026 expiry), larger than parity should allow. The likely culprit is quote noise and wide ITM spreads; treat single-strike IV readings from this snapshot with caution. The structural ITM divergence is on top of that:
...
ATM IV currently averages 11.1% for expiries within a week vs 12.1% around one month: an **upward-sloping (contango)** curve, the normal calm-market shape — near-term realized vol is expected to stay low, while longer horizons carry a risk premium for the things that haven't happened yet.
...
- **This snapshot uses live bid/ask midpoints** (market-hours pull): 369 zero-bid contracts dropped and 372 more for spreads wider than 20% of mid.
...
- **16.1% of IV solves failed** (367 contracts, mostly the shortest expiries and deep-ITM strikes); 157 were prices below intrinsic value — quotes that genuinely admit no implied vol.
