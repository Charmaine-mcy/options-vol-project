...
## Results (latest snapshot: 2026-07-21 15:02 ET, SPY spot 748.22)

| Stage | Count |
|---|---|
| Contracts pulled (12 expiries) | 3,225 |
| Survive liquidity filter | 2,357 (73%) |
| IV solved via Newton | 1,526 (65%) |
| IV solved via Brent fallback | 652 (28%) |
| Failed (no root / outside bounds) | 179 (7.6%) |
...
The 179 failures cluster in the shortest expiries and deep-ITM strikes; 174 of them are quotes sitting below intrinsic value — prices that genuinely admit no implied vol, which the solver refuses rather than forcing a number.
...
Put-call parity says call IV and put IV should be identical at the same strike — but in this snapshot they differ by 1.6 vol points even at the money (14.9% calls vs 13.3% puts on the 21 Aug 2026 expiry), larger than parity should allow. The likely culprit is quote noise and wide ITM spreads; treat single-strike IV readings from this snapshot with caution. The structural ITM divergence is on top of that:
...
ATM IV currently averages 13.7% for expiries within a week vs 14.1% around one month: an **upward-sloping (contango)** curve, the normal calm-market shape — near-term realized vol is expected to stay low, while longer horizons carry a risk premium for the things that haven't happened yet.
...
- **This snapshot uses live bid/ask midpoints** (market-hours pull): 506 zero-bid contracts dropped and 362 more for spreads wider than 20% of mid.
...
- **7.6% of IV solves failed** (179 contracts, mostly the shortest expiries and deep-ITM strikes); 174 were prices below intrinsic value — quotes that genuinely admit no implied vol.
