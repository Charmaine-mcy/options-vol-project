...
## Results (latest snapshot: 2026-08-24 11:11 ET, SPY spot 762.94)

| Stage | Count |
|---|---|
| Contracts pulled (12 expiries) | 3,613 |
| Survive liquidity filter | 2,587 (72%) |
| IV solved via Newton | 1,570 (61%) |
| IV solved via Brent fallback | 697 (27%) |
| Failed (no root / outside bounds) | 320 (12.4%) |
...
The 320 failures cluster in the shortest expiries and deep-ITM strikes; 318 of them are quotes sitting below intrinsic value — prices that genuinely admit no implied vol, which the solver refuses rather than forcing a number.
...
Under Black-Scholes assumptions the IV-vs-strike line would be flat. Instead SPY shows the classic **equity skew**: the 18 Sep 2026 expiry prices 13% at the money vs 24% for puts 10% below spot (+11.0 vol pts). Three standard explanations, all pushing the same direction:
...
ATM IV currently averages 12.6% for expiries within a week vs 13.2% around one month: an **upward-sloping (contango)** curve, the normal calm-market shape — near-term realized vol is expected to stay low, while longer horizons carry a risk premium for the things that haven't happened yet.
...
NFLX reports 20 Oct 2026 (57 days away) — the event is being priced *cross-sectionally* right now: the first post-earnings expiry (20 Nov) carries 39% ATM IV while later expiries decay back toward baseline as the one-day jump is diluted over more calendar time.
...
- **This snapshot uses live bid/ask midpoints** (market-hours pull): 448 zero-bid contracts dropped and 578 more for spreads wider than 20% of mid.
...
- **12.4% of IV solves failed** (320 contracts, mostly the shortest expiries and deep-ITM strikes); 318 were prices below intrinsic value — quotes that genuinely admit no implied vol.
