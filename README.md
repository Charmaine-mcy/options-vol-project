...
## Results (latest snapshot: 2026-07-31 09:39 ET, SPY spot 744.63)

| Stage | Count |
|---|---|
| Contracts pulled (12 expiries) | 3,620 |
| Survive liquidity filter | 2,423 (67%) |
| IV solved via Newton | 935 (39%) |
| IV solved via Brent fallback | 1,058 (44%) |
| Failed (no root / outside bounds) | 430 (17.7%) |
...
The 430 failures cluster in the shortest expiries and deep-ITM strikes; 163 of them are quotes sitting below intrinsic value — prices that genuinely admit no implied vol, which the solver refuses rather than forcing a number.
...
Under Black-Scholes assumptions the IV-vs-strike line would be flat. Instead SPY shows the classic **equity skew**: the 31 Aug 2026 expiry prices 14% at the money vs 24% for puts 10% below spot (+10.4 vol pts). Three standard explanations, all pushing the same direction:
...
ATM IV currently averages 13.4% for expiries within a week vs 14.5% around one month: an **upward-sloping (contango)** curve, the normal calm-market shape — near-term realized vol is expected to stay low, while longer horizons carry a risk premium for the things that haven't happened yet.
...
No earnings inside the tracking window at the moment — the next report is 20 Oct 2026 (~81 days away); tracking resumes automatically inside the 60-day window. The chart shows the **archived analysis of the 16 Jul 2026 event**, rendered from numbers persisted at capture time (`data/earnings_archive_NFLX.json`), not recomputed:
...
- **This snapshot was pulled outside market hours.** Yahoo zeroes bid/ask overnight, so prices are prior-session lastPrice with a volume filter (1,197 contracts dropped, 33%); midpoint data from market hours is strictly better.
...
- **17.7% of IV solves failed** (430 contracts, mostly the shortest expiries and deep-ITM strikes); 163 were prices below intrinsic value — quotes that genuinely admit no implied vol.
...
- **Discrete dividends** are approximated by a continuous yield (SPY q ~ 1.25% trailing); fine at this horizon, cruder for long-dated options.
