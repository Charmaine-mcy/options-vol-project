...
## Results (latest snapshot: 2026-08-18 11:03 ET, SPY spot 767.80)

| Stage | Count |
|---|---|
| Contracts pulled (12 expiries) | 3,396 |
| Survive liquidity filter | 2,562 (75%) |
| IV solved via Newton | 1,543 (60%) |
| IV solved via Brent fallback | 696 (27%) |
| Failed (no root / outside bounds) | 323 (12.6%) |
...
The 323 failures cluster in the shortest expiries and deep-ITM strikes; 232 of them are quotes sitting below intrinsic value — prices that genuinely admit no implied vol, which the solver refuses rather than forcing a number.
...
Under Black-Scholes assumptions the IV-vs-strike line would be flat. Instead SPY shows the classic **equity skew**: the 11 Sep 2026 expiry prices 13% at the money vs 24% for puts 10% below spot (+11.3 vol pts). Three standard explanations, all pushing the same direction:
...
Put-call parity says call IV and put IV should be identical at the same strike — but in this snapshot they differ by 1.8 vol points even at the money (13.5% calls vs 11.6% puts on the 11 Sep 2026 expiry), larger than parity should allow. The likely culprit is quote noise and wide ITM spreads; treat single-strike IV readings from this snapshot with caution. The structural ITM divergence is on top of that:
...
ATM IV currently averages 11.9% for expiries within a week vs 12.6% around one month: an **upward-sloping (contango)** curve, the normal calm-market shape — near-term realized vol is expected to stay low, while longer horizons carry a risk premium for the things that haven't happened yet.
...
- **This snapshot uses live bid/ask midpoints** (market-hours pull): 327 zero-bid contracts dropped and 507 more for spreads wider than 20% of mid.
...
- **12.6% of IV solves failed** (323 contracts, mostly the shortest expiries and deep-ITM strikes); 232 were prices below intrinsic value — quotes that genuinely admit no implied vol.
