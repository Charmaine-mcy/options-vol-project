...
## Results (latest snapshot: 2026-09-03 13:07 ET, SPY spot 772.61)

| Stage | Count |
|---|---|
| Contracts pulled (12 expiries) | 3,834 |
| Survive liquidity filter | 3,025 (79%) |
| IV solved via Newton | 2,015 (67%) |
| IV solved via Brent fallback | 875 (29%) |
| Failed (no root / outside bounds) | 135 (4.5%) |
...
The 135 failures cluster in the shortest expiries and deep-ITM strikes; 119 of them are quotes sitting below intrinsic value — prices that genuinely admit no implied vol, which the solver refuses rather than forcing a number.
...
Under Black-Scholes assumptions the IV-vs-strike line would be flat. Instead SPY shows the classic **equity skew**: the 02 Oct 2026 expiry prices 12% at the money vs 22% for puts 10% below spot (+10.2 vol pts). Three standard explanations, all pushing the same direction:
...
ATM IV currently averages 10.7% for expiries within a week vs 12.0% around one month: an **upward-sloping (contango)** curve, the normal calm-market shape — near-term realized vol is expected to stay low, while longer horizons carry a risk premium for the things that haven't happened yet.
...
Backing out the event variance gives a **market-implied earnings-day move of 13.2%** of spot (method: pre/post expiry variance difference). After the print, the front expiry's IV should collapse onto the baseline — the "vol crush" — which the pipeline captures automatically in its post-event grace window.
<!-- AUTO:EARNINGS:END -->
...
- **This snapshot uses live bid/ask midpoints** (market-hours pull): 406 zero-bid contracts dropped and 403 more for spreads wider than 20% of mid.
...
- **4.5% of IV solves failed** (135 contracts, mostly the shortest expiries and deep-ITM strikes); 119 were prices below intrinsic value — quotes that genuinely admit no implied vol.
