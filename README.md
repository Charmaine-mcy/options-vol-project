...
## Results (latest snapshot: 2026-09-25 14:49 ET, SPY spot 770.89)

| Stage | Count |
|---|---|
| Contracts pulled (12 expiries) | 3,477 |
| Survive liquidity filter | 2,642 (76%) |
| IV solved via Newton | 1,728 (65%) |
| IV solved via Brent fallback | 682 (26%) |
| Failed (no root / outside bounds) | 232 (8.8%) |
...
The 232 failures cluster in the shortest expiries and deep-ITM strikes; 125 of them are quotes sitting below intrinsic value — prices that genuinely admit no implied vol, which the solver refuses rather than forcing a number.
...
Under Black-Scholes assumptions the IV-vs-strike line would be flat. Instead SPY shows the classic **equity skew**: the 23 Oct 2026 expiry prices 12% at the money vs 23% for puts 10% below spot (+10.3 vol pts). Three standard explanations, all pushing the same direction:
...
ATM IV currently averages 11.0% for expiries within a week vs 12.4% around one month: an **upward-sloping (contango)** curve, the normal calm-market shape — near-term realized vol is expected to stay low, while longer horizons carry a risk premium for the things that haven't happened yet.
...
- **This snapshot uses live bid/ask midpoints** (market-hours pull): 434 zero-bid contracts dropped and 401 more for spreads wider than 20% of mid.
...
- **8.8% of IV solves failed** (232 contracts, mostly the shortest expiries and deep-ITM strikes); 125 were prices below intrinsic value — quotes that genuinely admit no implied vol.
