...
## Results (latest snapshot: 2026-07-29 15:04 ET, SPY spot 740.39)

| Stage | Count |
|---|---|
| Contracts pulled (12 expiries) | 3,374 |
| Survive liquidity filter | 2,569 (76%) |
| IV solved via Newton | 1,521 (59%) |
| IV solved via Brent fallback | 726 (28%) |
| Failed (no root / outside bounds) | 322 (12.5%) |
...
The 322 failures cluster in the shortest expiries and deep-ITM strikes; 312 of them are quotes sitting below intrinsic value — prices that genuinely admit no implied vol, which the solver refuses rather than forcing a number.
...
Under Black-Scholes assumptions the IV-vs-strike line would be flat. Instead SPY shows the classic **equity skew**: the 28 Aug 2026 expiry prices 15% at the money vs 25% for puts 10% below spot (+9.5 vol pts). Three standard explanations, all pushing the same direction:
...
Put-call parity says call IV and put IV should be identical at the same strike — but in this snapshot they differ by 2.7 vol points even at the money (16.4% calls vs 13.7% puts on the 28 Aug 2026 expiry), larger than parity should allow. The likely culprit is quote noise and wide ITM spreads; treat single-strike IV readings from this snapshot with caution. The structural ITM divergence is on top of that:
...
- **This snapshot uses live bid/ask midpoints** (market-hours pull): 519 zero-bid contracts dropped and 286 more for spreads wider than 20% of mid.
...
- **12.5% of IV solves failed** (322 contracts, mostly the shortest expiries and deep-ITM strikes); 312 were prices below intrinsic value — quotes that genuinely admit no implied vol.
