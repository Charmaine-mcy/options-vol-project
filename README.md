...
## Results (latest snapshot: 2026-08-17 13:57 ET, SPY spot 773.37)

| Stage | Count |
|---|---|
| Contracts pulled (12 expiries) | 3,328 |
| Survive liquidity filter | 2,433 (73%) |
| IV solved via Newton | 1,607 (66%) |
| IV solved via Brent fallback | 616 (25%) |
| Failed (no root / outside bounds) | 210 (8.6%) |
...
The 210 failures cluster in the shortest expiries and deep-ITM strikes; 109 of them are quotes sitting below intrinsic value — prices that genuinely admit no implied vol, which the solver refuses rather than forcing a number.
...
Under Black-Scholes assumptions the IV-vs-strike line would be flat. Instead SPY shows the classic **equity skew**: the 04 Sep 2026 expiry prices 12% at the money vs 25% for puts 10% below spot (+12.9 vol pts). Three standard explanations, all pushing the same direction:
...
ATM IV currently averages 10.6% for expiries within a week vs nan% around one month: an **essentially flat** curve — neither calm-market contango nor stress backwardation. Flat term structures are transitional; watch whether the short end lifts (event risk approaching) or the long end reasserts the usual premium.
...
No earnings inside the tracking window at the moment — the next report is 20 Oct 2026 (~64 days away); tracking resumes automatically inside the 60-day window. The chart shows the **archived analysis of the 16 Jul 2026 event**, rendered from numbers persisted at capture time (`data/earnings_archive_NFLX.json`), not recomputed:
...
- **This snapshot uses live bid/ask midpoints** (market-hours pull): 378 zero-bid contracts dropped and 517 more for spreads wider than 20% of mid.
...
- **8.6% of IV solves failed** (210 contracts, mostly the shortest expiries and deep-ITM strikes); 109 were prices below intrinsic value — quotes that genuinely admit no implied vol.
