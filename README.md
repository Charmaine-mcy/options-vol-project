...
## Results (latest snapshot: 2026-07-31 10:48 ET, SPY spot 741.96)

| Stage | Count |
|---|---|
| Contracts pulled (12 expiries) | 3,620 |
| Survive liquidity filter | 2,892 (80%) |
| IV solved via Newton | 1,595 (55%) |
| IV solved via Brent fallback | 876 (30%) |
| Failed (no root / outside bounds) | 421 (14.6%) |
...
The 421 failures cluster in the shortest expiries and deep-ITM strikes; 326 of them are quotes sitting below intrinsic value — prices that genuinely admit no implied vol, which the solver refuses rather than forcing a number.
...
Under Black-Scholes assumptions the IV-vs-strike line would be flat. Instead SPY shows the classic **equity skew**: the 31 Aug 2026 expiry prices 14% at the money vs 24% for puts 10% below spot (+10.0 vol pts). Three standard explanations, all pushing the same direction:
...
- **This snapshot uses live bid/ask midpoints** (market-hours pull): 418 zero-bid contracts dropped and 310 more for spreads wider than 20% of mid.
...
- **14.6% of IV solves failed** (421 contracts, mostly the shortest expiries and deep-ITM strikes); 326 were prices below intrinsic value — quotes that genuinely admit no implied vol.
