...
## Results (latest snapshot: 2026-08-07 10:34 ET, SPY spot 772.55)

| Stage | Count |
|---|---|
| Contracts pulled (12 expiries) | 3,486 |
| Survive liquidity filter | 2,778 (80%) |
| IV solved via Newton | 1,008 (36%) |
| IV solved via Brent fallback | 911 (33%) |
| Failed (no root / outside bounds) | 859 (30.9%) |
...
The 859 failures cluster in the shortest expiries and deep-ITM strikes; 665 of them are quotes sitting below intrinsic value — prices that genuinely admit no implied vol, which the solver refuses rather than forcing a number.
...
Under Black-Scholes assumptions the IV-vs-strike line would be flat. Instead SPY shows the classic **equity skew**: the 04 Sep 2026 expiry prices 12% at the money vs 22% for puts 10% below spot (+9.8 vol pts). Three standard explanations, all pushing the same direction:
...
ATM IV currently averages 10.9% for expiries within a week vs 12.4% around one month: an **upward-sloping (contango)** curve, the normal calm-market shape — near-term realized vol is expected to stay low, while longer horizons carry a risk premium for the things that haven't happened yet.
...
No earnings inside the tracking window at the moment — the next report is 20 Oct 2026 (~74 days away); tracking resumes automatically inside the 60-day window. The chart shows the **archived analysis of the 16 Jul 2026 event**, rendered from numbers persisted at capture time (`data/earnings_archive_NFLX.json`), not recomputed:
...
- **This snapshot uses live bid/ask midpoints** (market-hours pull): 293 zero-bid contracts dropped and 415 more for spreads wider than 20% of mid.
...
- **30.9% of IV solves failed** (859 contracts, mostly the shortest expiries and deep-ITM strikes); 665 were prices below intrinsic value — quotes that genuinely admit no implied vol.
