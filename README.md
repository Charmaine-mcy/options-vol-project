...
## Results (latest snapshot: 2026-08-19 10:07 ET, SPY spot 769.73)

| Stage | Count |
|---|---|
| Contracts pulled (12 expiries) | 3,766 |
| Survive liquidity filter | 2,896 (77%) |
| IV solved via Newton | 1,398 (48%) |
| IV solved via Brent fallback | 811 (28%) |
| Failed (no root / outside bounds) | 687 (23.7%) |
...
The 687 failures cluster in the shortest expiries and deep-ITM strikes; 457 of them are quotes sitting below intrinsic value — prices that genuinely admit no implied vol, which the solver refuses rather than forcing a number.
...
Under Black-Scholes assumptions the IV-vs-strike line would be flat. Instead SPY shows the classic **equity skew**: the 18 Sep 2026 expiry prices 13% at the money vs 24% for puts 10% below spot (+10.6 vol pts). Three standard explanations, all pushing the same direction:
...
ATM IV currently averages 12.2% for expiries within a week vs 12.7% around one month: an **upward-sloping (contango)** curve, the normal calm-market shape — near-term realized vol is expected to stay low, while longer horizons carry a risk premium for the things that haven't happened yet.
...
- **This snapshot uses live bid/ask midpoints** (market-hours pull): 365 zero-bid contracts dropped and 505 more for spreads wider than 20% of mid.
...
- **23.7% of IV solves failed** (687 contracts, mostly the shortest expiries and deep-ITM strikes); 457 were prices below intrinsic value — quotes that genuinely admit no implied vol.
