...
## Results (latest snapshot: 2026-07-31 11:22 ET, SPY spot 741.45)

| Stage | Count |
|---|---|
| Contracts pulled (12 expiries) | 3,620 |
| Survive liquidity filter | 2,890 (80%) |
| IV solved via Newton | 1,731 (60%) |
| IV solved via Brent fallback | 920 (32%) |
| Failed (no root / outside bounds) | 239 (8.3%) |
...
The 239 failures cluster in the shortest expiries and deep-ITM strikes; 199 of them are quotes sitting below intrinsic value — prices that genuinely admit no implied vol, which the solver refuses rather than forcing a number.
...
Under Black-Scholes assumptions the IV-vs-strike line would be flat. Instead SPY shows the classic **equity skew**: the 31 Aug 2026 expiry prices 15% at the money vs 24% for puts 10% below spot (+9.4 vol pts). Three standard explanations, all pushing the same direction:
...
Put-call parity says call IV and put IV should be identical at the same strike — but in this snapshot they differ by 2.0 vol points even at the money (15.5% calls vs 13.5% puts on the 31 Aug 2026 expiry), larger than parity should allow. The likely culprit is quote noise and wide ITM spreads; treat single-strike IV readings from this snapshot with caution. The structural ITM divergence is on top of that:
...
ATM IV currently averages 15.0% for expiries within a week vs 14.7% around one month: an **essentially flat** curve — neither calm-market contango nor stress backwardation. Flat term structures are transitional; watch whether the short end lifts (event risk approaching) or the long end reasserts the usual premium.
...
- **This snapshot uses live bid/ask midpoints** (market-hours pull): 414 zero-bid contracts dropped and 316 more for spreads wider than 20% of mid.
...
- **8.3% of IV solves failed** (239 contracts, mostly the shortest expiries and deep-ITM strikes); 199 were prices below intrinsic value — quotes that genuinely admit no implied vol.
