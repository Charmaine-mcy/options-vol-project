...
## Results (latest snapshot: 2026-08-14 11:23 ET, SPY spot 776.21)

| Stage | Count |
|---|---|
| Contracts pulled (12 expiries) | 3,857 |
| Survive liquidity filter | 2,979 (77%) |
| IV solved via Newton | 2,004 (67%) |
| IV solved via Brent fallback | 814 (27%) |
| Failed (no root / outside bounds) | 161 (5.4%) |
...
The 161 failures cluster in the shortest expiries and deep-ITM strikes; 146 of them are quotes sitting below intrinsic value — prices that genuinely admit no implied vol, which the solver refuses rather than forcing a number.
...
Under Black-Scholes assumptions the IV-vs-strike line would be flat. Instead SPY shows the classic **equity skew**: the 11 Sep 2026 expiry prices 12% at the money vs 22% for puts 10% below spot (+10.2 vol pts). Three standard explanations, all pushing the same direction:
...
The upturn in the far call wing (14% at 10% above spot) completes the "smirk" — lottery-ticket demand for upside calls, plus the fact that a big enough rally is also a volatility event.
<!-- AUTO:SMILE:END -->
...
Put-call parity says call IV and put IV should be identical at the same strike — but in this snapshot they differ by 1.7 vol points even at the money (12.9% calls vs 11.2% puts on the 11 Sep 2026 expiry), larger than parity should allow. The likely culprit is quote noise and wide ITM spreads; treat single-strike IV readings from this snapshot with caution. The structural ITM divergence is on top of that:
...
ATM IV currently averages 8.5% for expiries within a week vs 12.4% around one month: an **upward-sloping (contango)** curve, the normal calm-market shape — near-term realized vol is expected to stay low, while longer horizons carry a risk premium for the things that haven't happened yet.
...
No earnings inside the tracking window at the moment — the next report is 20 Oct 2026 (~67 days away); tracking resumes automatically inside the 60-day window. The chart shows the **archived analysis of the 16 Jul 2026 event**, rendered from numbers persisted at capture time (`data/earnings_archive_NFLX.json`), not recomputed:
...
- **This snapshot uses live bid/ask midpoints** (market-hours pull): 387 zero-bid contracts dropped and 491 more for spreads wider than 20% of mid.
...
- **5.4% of IV solves failed** (161 contracts, mostly the shortest expiries and deep-ITM strikes); 146 were prices below intrinsic value — quotes that genuinely admit no implied vol.
...
- **Discrete dividends** are approximated by a continuous yield (SPY q ~ 1.20% trailing); fine at this horizon, cruder for long-dated options.
