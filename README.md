...
## Results (latest snapshot: 2026-08-24 10:12 ET, SPY spot 762.65)

| Stage | Count |
|---|---|
| Contracts pulled (12 expiries) | 3,613 |
| Survive liquidity filter | 2,570 (71%) |
| IV solved via Newton | 1,548 (60%) |
| IV solved via Brent fallback | 695 (27%) |
| Failed (no root / outside bounds) | 327 (12.7%) |
...
The 327 failures cluster in the shortest expiries and deep-ITM strikes; 324 of them are quotes sitting below intrinsic value — prices that genuinely admit no implied vol, which the solver refuses rather than forcing a number.
...
Under Black-Scholes assumptions the IV-vs-strike line would be flat. Instead SPY shows the classic **equity skew**: the 18 Sep 2026 expiry prices 13% at the money vs 24% for puts 10% below spot (+10.9 vol pts). Three standard explanations, all pushing the same direction:
...
NFLX reports 20 Oct 2026 (57 days away) — the event is being priced *cross-sectionally* right now: the first post-earnings expiry (20 Nov) carries 38% ATM IV while later expiries decay back toward baseline as the one-day jump is diluted over more calendar time.
...
- **This snapshot uses live bid/ask midpoints** (market-hours pull): 449 zero-bid contracts dropped and 594 more for spreads wider than 20% of mid.
...
- **12.7% of IV solves failed** (327 contracts, mostly the shortest expiries and deep-ITM strikes); 324 were prices below intrinsic value — quotes that genuinely admit no implied vol.
