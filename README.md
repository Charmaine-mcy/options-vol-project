...
## Results (latest snapshot: 2026-07-24 10:43 ET, SPY spot 738.25)

| Stage | Count |
|---|---|
| Contracts pulled (12 expiries) | 2,982 |
| Survive liquidity filter | 2,218 (74%) |
| IV solved via Newton | 1,463 (66%) |
| IV solved via Brent fallback | 555 (25%) |
| Failed (no root / outside bounds) | 200 (9.0%) |
...
The 200 failures cluster in the shortest expiries and deep-ITM strikes; 152 of them are quotes sitting below intrinsic value — prices that genuinely admit no implied vol, which the solver refuses rather than forcing a number.
...
Under Black-Scholes assumptions the IV-vs-strike line would be flat. Instead SPY shows the classic **equity skew**: the 14 Aug 2026 expiry prices 16% at the money vs 28% for puts 10% below spot (+11.8 vol pts). Three standard explanations, all pushing the same direction:
...
ATM IV currently averages 15.5% for expiries within a week vs 15.8% around one month: an **upward-sloping (contango)** curve, the normal calm-market shape — near-term realized vol is expected to stay low, while longer horizons carry a risk premium for the things that haven't happened yet.
...
- **This snapshot uses live bid/ask midpoints** (market-hours pull): 471 zero-bid contracts dropped and 293 more for spreads wider than 20% of mid.
...
- **9.0% of IV solves failed** (200 contracts, mostly the shortest expiries and deep-ITM strikes); 152 were prices below intrinsic value — quotes that genuinely admit no implied vol.
