...
## Results (latest snapshot: 2026-08-14 10:32 ET, SPY spot 776.89)

| Stage | Count |
|---|---|
| Contracts pulled (12 expiries) | 3,857 |
| Survive liquidity filter | 3,002 (78%) |
| IV solved via Newton | 1,932 (64%) |
| IV solved via Brent fallback | 836 (28%) |
| Failed (no root / outside bounds) | 234 (7.8%) |
...
The 234 failures cluster in the shortest expiries and deep-ITM strikes; 220 of them are quotes sitting below intrinsic value — prices that genuinely admit no implied vol, which the solver refuses rather than forcing a number.
...
Under Black-Scholes assumptions the IV-vs-strike line would be flat. Instead SPY shows the classic **equity skew**: the 11 Sep 2026 expiry prices 12% at the money vs 22% for puts 10% below spot (+10.0 vol pts). Three standard explanations, all pushing the same direction:
...
Put-call parity says call IV and put IV should be identical at the same strike — but in this snapshot they differ by 2.4 vol points even at the money (13.4% calls vs 11.0% puts on the 11 Sep 2026 expiry), larger than parity should allow. The likely culprit is quote noise and wide ITM spreads; treat single-strike IV readings from this snapshot with caution. The structural ITM divergence is on top of that:
...
ATM IV currently averages 9.2% for expiries within a week vs 12.6% around one month: an **upward-sloping (contango)** curve, the normal calm-market shape — near-term realized vol is expected to stay low, while longer horizons carry a risk premium for the things that haven't happened yet.
...
- **This snapshot uses live bid/ask midpoints** (market-hours pull): 389 zero-bid contracts dropped and 466 more for spreads wider than 20% of mid.
...
- **7.8% of IV solves failed** (234 contracts, mostly the shortest expiries and deep-ITM strikes); 220 were prices below intrinsic value — quotes that genuinely admit no implied vol.
