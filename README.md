...
## Results (latest snapshot: 2026-09-11 13:07 ET, SPY spot 765.66)

| Stage | Count |
|---|---|
| Contracts pulled (12 expiries) | 3,121 |
| Survive liquidity filter | 2,484 (80%) |
| IV solved via Newton | 1,155 (46%) |
| IV solved via Brent fallback | 721 (29%) |
| Failed (no root / outside bounds) | 608 (24.5%) |
...
The 608 failures cluster in the shortest expiries and deep-ITM strikes; 217 of them are quotes sitting below intrinsic value — prices that genuinely admit no implied vol, which the solver refuses rather than forcing a number.
...
Under Black-Scholes assumptions the IV-vs-strike line would be flat. Instead SPY shows the classic **equity skew**: the 09 Oct 2026 expiry prices 13% at the money vs 24% for puts 10% below spot (+11.2 vol pts). Three standard explanations, all pushing the same direction:
...
NFLX reports 20 Oct 2026 (39 days away) — the event is being priced *cross-sectionally* right now: the first post-earnings expiry (23 Oct) carries 41% ATM IV while later expiries decay back toward baseline as the one-day jump is diluted over more calendar time.
...
- **This snapshot uses live bid/ask midpoints** (market-hours pull): 317 zero-bid contracts dropped and 320 more for spreads wider than 20% of mid.
...
- **24.5% of IV solves failed** (608 contracts, mostly the shortest expiries and deep-ITM strikes); 217 were prices below intrinsic value — quotes that genuinely admit no implied vol.
