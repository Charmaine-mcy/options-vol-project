...
## Results (latest snapshot: 2026-07-22 11:43 ET, SPY spot 748.89)

| Stage | Count |
|---|---|
| Contracts pulled (12 expiries) | 3,194 |
| Survive liquidity filter | 2,394 (75%) |
| IV solved via Newton | 1,539 (64%) |
| IV solved via Brent fallback | 687 (29%) |
| Failed (no root / outside bounds) | 168 (7.0%) |
...
The 168 failures cluster in the shortest expiries and deep-ITM strikes; 163 of them are quotes sitting below intrinsic value — prices that genuinely admit no implied vol, which the solver refuses rather than forcing a number.
...
Put-call parity says call IV and put IV should be identical at the same strike — but in this snapshot they differ by 2.0 vol points even at the money (15.0% calls vs 13.0% puts on the 21 Aug 2026 expiry), larger than parity should allow. The likely culprit is quote noise and wide ITM spreads; treat single-strike IV readings from this snapshot with caution. The structural ITM divergence is on top of that:
...
ATM IV currently averages 13.8% for expiries within a week vs 14.1% around one month: an **upward-sloping (contango)** curve, the normal calm-market shape — near-term realized vol is expected to stay low, while longer horizons carry a risk premium for the things that haven't happened yet.
...
- **This snapshot uses live bid/ask midpoints** (market-hours pull): 412 zero-bid contracts dropped and 388 more for spreads wider than 20% of mid.
...
- **7.0% of IV solves failed** (168 contracts, mostly the shortest expiries and deep-ITM strikes); 163 were prices below intrinsic value — quotes that genuinely admit no implied vol.
