...
## Results (latest snapshot: 2026-07-21 09:30 ET, SPY spot 746.37)

| Stage | Count |
|---|---|
| Contracts pulled (12 expiries) | 3,225 |
| Survive liquidity filter | 2,071 (64%) |
| IV solved via Newton | 801 (39%) |
| IV solved via Brent fallback | 1,031 (50%) |
| Failed (no root / outside bounds) | 239 (11.5%) |

Brent carries most of the load in this snapshot — typical when short-dated expiries or noisy wing quotes dominate (vega shrinks with sqrt(T), so the Newton step explodes where the price barely responds to vol).
The 239 failures cluster in the shortest expiries and deep-ITM strikes; 112 of them are quotes sitting below intrinsic value — prices that genuinely admit no implied vol, which the solver refuses rather than forcing a number.
...
Under Black-Scholes assumptions the IV-vs-strike line would be flat. Instead SPY shows the classic **equity skew**: the 21 Aug 2026 expiry prices 15% at the money vs 26% for puts 10% below spot (+10.6 vol pts). Three standard explanations, all pushing the same direction:
...
The far call wing is currently flat-to-soft (13% at 10% above spot) — no meaningful lottery-ticket bid for upside calls in this snapshot.
<!-- AUTO:SMILE:END -->
...
Put-call parity says call IV and put IV should be identical at the same strike — but in this snapshot they differ by 3.8 vol points even at the money (13.1% calls vs 16.9% puts on the 21 Aug 2026 expiry), larger than parity should allow. The likely culprit is staleness in last-trade prices (overnight snapshot); treat single-strike IV readings from this snapshot with caution. The structural ITM divergence is on top of that:

- An ITM option's price is nearly all intrinsic value; the vol information lives in a few cents of time value, and staleness in last-trade prices (overnight snapshot) can swing ITM implied vols enormously. (OTM prices are *pure* time value — much more informative per cent of noise.)
...
ATM IV currently averages 17.8% for expiries within a week vs 15.0% around one month: a **downward-sloping (backwardation)** curve — the market is pricing *more* volatility near-term than long-term. That is the stressed shape: either a known near-dated event (macro print, earnings cluster) or a recent vol spike that the market expects to mean-revert. Worth checking the calendar before reading it as generalized fear.
...
No earnings inside the tracking window at the moment — the next report is 20 Oct 2026 (~91 days away); tracking resumes automatically inside the 60-day window. The chart shows the **archived analysis of the 16 Jul 2026 event**, rendered from numbers persisted at capture time (`data/earnings_archive_NFLX.json`), not recomputed:
...
- **This snapshot was pulled outside market hours.** Yahoo zeroes bid/ask overnight, so prices are prior-session lastPrice with a volume filter (1,154 contracts dropped, 36%); midpoint data from market hours is strictly better.
- **yfinance's own `impliedVolatility` column is garbage on overnight pulls** (~1e-5 across the chain when observed) — a good illustration of why this project solves for IV itself.
- **11.5% of IV solves failed** (239 contracts, mostly the shortest expiries and deep-ITM strikes); 112 were prices below intrinsic value — quotes that genuinely admit no implied vol.
