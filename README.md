...
## Results (latest snapshot: 2026-08-24 13:06 ET, SPY spot 764.08)

| Stage | Count |
|---|---|
| Contracts pulled (12 expiries) | 3,613 |
| Survive liquidity filter | 2,581 (71%) |
| IV solved via Newton | 1,714 (66%) |
| IV solved via Brent fallback | 687 (27%) |
| Failed (no root / outside bounds) | 180 (7.0%) |
...
The 180 failures cluster in the shortest expiries and deep-ITM strikes; 160 of them are quotes sitting below intrinsic value — prices that genuinely admit no implied vol, which the solver refuses rather than forcing a number.
...
Under Black-Scholes assumptions the IV-vs-strike line would be flat. Instead SPY shows the classic **equity skew**: the 18 Sep 2026 expiry prices 13% at the money vs 24% for puts 10% below spot (+11.1 vol pts). Three standard explanations, all pushing the same direction:
...
ATM IV currently averages 12.1% for expiries within a week vs 13.0% around one month: an **upward-sloping (contango)** curve, the normal calm-market shape — near-term realized vol is expected to stay low, while longer horizons carry a risk premium for the things that haven't happened yet.
...
- **This snapshot uses live bid/ask midpoints** (market-hours pull): 459 zero-bid contracts dropped and 573 more for spreads wider than 20% of mid.
...
- **7.0% of IV solves failed** (180 contracts, mostly the shortest expiries and deep-ITM strikes); 160 were prices below intrinsic value — quotes that genuinely admit no implied vol.
