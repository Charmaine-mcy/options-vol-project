...
## Results (latest snapshot: 2026-08-19 11:03 ET, SPY spot 772.07)

| Stage | Count |
|---|---|
| Contracts pulled (12 expiries) | 3,766 |
| Survive liquidity filter | 2,901 (77%) |
| IV solved via Newton | 1,374 (47%) |
| IV solved via Brent fallback | 815 (28%) |
| Failed (no root / outside bounds) | 712 (24.5%) |
...
The 712 failures cluster in the shortest expiries and deep-ITM strikes; 498 of them are quotes sitting below intrinsic value — prices that genuinely admit no implied vol, which the solver refuses rather than forcing a number.
...
Under Black-Scholes assumptions the IV-vs-strike line would be flat. Instead SPY shows the classic **equity skew**: the 18 Sep 2026 expiry prices 13% at the money vs 23% for puts 10% below spot (+10.5 vol pts). Three standard explanations, all pushing the same direction:
...
ATM IV currently averages 10.7% for expiries within a week vs 12.2% around one month: an **upward-sloping (contango)** curve, the normal calm-market shape — near-term realized vol is expected to stay low, while longer horizons carry a risk premium for the things that haven't happened yet.
...
No earnings inside the tracking window at the moment — the next report is 20 Oct 2026 (~62 days away); tracking resumes automatically inside the 60-day window. The chart shows the **archived analysis of the 16 Jul 2026 event**, rendered from numbers persisted at capture time (`data/earnings_archive_NFLX.json`), not recomputed:
...
- **This snapshot uses live bid/ask midpoints** (market-hours pull): 348 zero-bid contracts dropped and 517 more for spreads wider than 20% of mid.
...
- **24.5% of IV solves failed** (712 contracts, mostly the shortest expiries and deep-ITM strikes); 498 were prices below intrinsic value — quotes that genuinely admit no implied vol.
